...
def component_hierarchy_example():
    ...
    optical_ports = [op for op in api.optical_ports if op["componentId"] in [c['id'] for c in circuits]]
    ...
    for reticle in reticles:
        ...
        for die in [d for d in dies if d['reticleId'] == reticle['id']]:
            ...
            for circuit in [c for c in circuits if c['dieId'] == die['id']]:
                ...
                for port in [op for op in optical_ports if op['componentId'] == circuit['id']]:
                    print(4*tab, "Optical Port:", port['name'], f"({port['position']})")
